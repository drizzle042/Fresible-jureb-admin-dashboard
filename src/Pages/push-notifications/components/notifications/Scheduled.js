...
const Scheduled = ({ styles, data }) => {

  const scheduledNotifications = data?.data?.filter(i => i?.scheduled === true);
  ...
  return (
    <div>
      ...
      {data && <PaginatorTemplate totalDocs={scheduledNotifications?.length} limit={data?.limit} page={data?.page} totalPages={data?.totalPages} />}
    </div>
  );
};
