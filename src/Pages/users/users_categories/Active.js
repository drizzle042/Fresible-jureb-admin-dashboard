    ...
    <div>
      <TableContainer className={styles.table_container} component={Paper}>
        <Table sx={{ minWidth: 650 }} aria-label="simple table">
          ...
          <TableBody>
            {activeAdministrators?.map((user, index) => (
              <TableRow
                key={index}
      ...
      {data && <PaginatorTemplate totalDocs={activeAdministrators?.length} limit={data?.limit} page={data?.page} totalPages={data?.totalPages} />}
    </div>
  );
};
