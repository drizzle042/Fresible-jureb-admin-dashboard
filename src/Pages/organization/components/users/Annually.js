...
const Annually = ({ styles }) => {

  // Pagination
  const { pageNumber, PaginatorTemplate } = usePaginator();

  const { data, isLoading, error } = useFetch(`${process.env.REACT_APP_BACKEND_API_URL}/api/v1/admin/cp/organizations?page=${pageNumber}&plan=ANNUALLY`)
